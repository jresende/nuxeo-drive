...
class TestVersioning(UnitTestCase):
    ...
    def test_versioning(self):
        ...
        assert self.remote_document_client_2.exists('/Test versioning.txt')
        doc = self.root_remote_client.fetch(TEST_WORKSPACE_PATH + '/Test versioning.txt')
        assert doc['properties']['uid:major_version'] == 0
        assert doc['properties']['uid:minor_version'] == 0
        ...
        doc = self.root_remote_client.fetch(TEST_WORKSPACE_PATH + '/Test versioning.txt')
        assert doc['properties']['uid:major_version'] == 0
        assert doc['properties']['uid:minor_version'] == 1
    ...
    def test_version_restore(self):
        ...
        self.wait_sync(wait_for_async=True)
        assert local_client.exists('/Document to restore.txt')
        assert local_client.get_content('/Document to restore.txt') ==  'Initial content.'
        ...
        self.wait_sync(wait_for_async=True)
        assert local_client.get_content('/Document to restore.txt') == 'Updated content.'
