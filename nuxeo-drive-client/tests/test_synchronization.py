...
class TestSynchronization(UnitTestCase):
    ...
    def test_binding_initialization_and_first_sync(self):
        ...
        assert local.get_content('/Folder 1/File 1.txt') == 'aaa'
        assert local.exists('/Folder 1/Folder 1.1')
        assert local.get_content('/Folder 1/Folder 1.1/File 2.txt') == 'bbb'
        assert local.exists('/Folder 1/Folder 1.2')
        assert local.get_content('/Folder 1/Folder 1.2/File 3.txt') == 'ccc'
        assert local.exists('/Folder 2')
        # Cannot predict the resolution in advance
        assert remote.get_content(self._duplicate_file_1)
        assert remote.get_content(self._duplicate_file_2)
        assert local.get_content('/Folder 2/File 4.txt') == 'ddd'
        assert local.get_content('/File 5.txt') == 'eee'
    ...
    def test_binding_synchronization_empty_start(self):
        ...
        assert remote.get_content('/Folder 1/File 1.txt') == 'aaaa'
        assert remote.exists('/Folder 4')

        assert local.get_content('/Folder 1/Folder 1.1/File 2.txt') == 'bbbb'
        # Let's just check remote document hasn't changed
        assert remote.get_content('/Folder 1/Folder 1.1/File 2.txt') == 'bbbb'
        assert not local.exists('/Folder 2')
        assert local.exists('/Folder 3')
        assert local.get_content('/Folder 3/File 6.txt') == 'ffff'
        ...
        assert remote.get_content('/Folder 1/File 1.txt') == '\x80'
        assert local.get_content('/Folder 1/Folder 1.1/File 2.txt') == '\x80'
        # Let's just check remote document hasn't changed
        assert remote.get_content('/Folder 1/Folder 1.1/File 2.txt') == '\x80'
    ...
    def test_synchronization_give_up(self):
        ...
        self.wait_sync()
        assert not self.engine_1.get_dao().get_errors(limit=test_error_threshold)
        workspace_children = self.engine_1.get_dao().get_states_from_partial_local('/' + self.workspace_title + '/')
        assert len(workspace_children) == 4
        for state in workspace_children:
            assert state.pair_state  == 'synchronized'
    ...
    def test_conflict_detection(self):
        ...
        assert local.get_content(local_path) == 'Same new content.'
        ...
        assert remote_1.get_content('/Some File.doc') == 'Same new content.'
        ...
        assert len(local_children) == 1
        assert local_children[0].name == 'Some File.doc'
        assert local.get_content(local_path) == 'Local new content.'
        remote_children = remote_1.get_children_info(self.workspace)
        assert len(remote_children) == 1
        assert remote_children[0].filename == 'Some File.doc'
        assert remote_1.get_content('/Some File.doc') == 'Remote new content.'
    ...
    def test_synchronize_deep_folders(self):
        ...
        assert local.exists(expected_folder_path)
        assert local.exists(expected_file_path)
        assert local.get_content(expected_file_path) == 'Fake non-zero content.'
        ...
        assert not local.exists(expected_folder_path)
        assert not local.exists(expected_file_path)
    ...
    def test_create_content_in_readonly_area(self):
        ...
        assert not sorted_states[0].local_name
        ...
        assert sorted_states[2].pair_state in ('locally_created', 'unsynchronized')
        assert sorted_states[3].local_name == 'Sub Folder 1'
        assert sorted_states[3].pair_state in ('locally_created', 'unsynchronized')
        assert sorted_states[4].local_name == 'File 2.txt'
        assert sorted_states[4].pair_state in ('locally_created', 'unsynchronized')
        ...
        assert sorted_states[5].pair_state in ('locally_created', 'unsynchronized')
        ...
        remote = self.remote_document_client_1
        assert remote.exists(u'/Readonly folder')
        ...
        self.wait_sync()
        assert not remote.exists(u'/Readonly folder/File in readonly folder')
        assert not remote.exists(u'/Readonly folder/Folder in readonly folder')
    ...
    def test_synchronize_special_filenames(self):
        ...
        assert folder_names == [u'Folder with chars- - - - - - - -']
        ...
        assert file_names == [u'File with chars- - - - - - - -.txt']
        ...
        assert local.get_content(u'/Folder with chars- - - - - - - -/File with chars- - - - - - - -.txt') == 'new content'
        ...
        self.wait_sync(wait_for_async=True, enforce_errors=False)
        file_names = [i.name for i in local.get_children_info(local.get_children_info('/')[0].path)]
        assert file_names == [u'File with chars- - - - - - - - - 2.txt']
    ...
    def test_synchronize_windows_foldername_endswith_space(self):
        ...
        self.engine_1.start()
        self.wait_sync(wait_for_async=True)
        assert self.local_root_client_1.exists('/Nuxeo Drive Test Workspace')
        if sys.platform == 'win32':
            assert self.local_root_client_1.exists('/Nuxeo Drive Test Workspace/trial/')
            assert self.local_root_client_1.exists('/Nuxeo Drive Test Workspace/trial/aFile.txt')
            assert self.local_root_client_1.exists('/Nuxeo Drive Test Workspace/trial/bFile.txt')
        else:
            assert self.local_root_client_1.exists('/Nuxeo Drive Test Workspace/trial /')
            assert self.local_root_client_1.exists('/Nuxeo Drive Test Workspace/trial /aFile.txt')
            assert self.local_root_client_1.exists('/Nuxeo Drive Test Workspace/trial /bFile.txt')
    ...
    def test_rename_and_create_same_folder_not_running(self):
        ...
        assert local_1.get_content('/Folder01/subfolder01/File02.txt'), b'42.42')
        """
