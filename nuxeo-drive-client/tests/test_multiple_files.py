...
class MultipleFilesTestCase(UnitTestCase):
    ...
    def setUp(self):
        ...
        for file_num in range(1, self.NUMBER_OF_LOCAL_FILES + 1):
            self.local_client_1.make_file(
                self.folder_path_1, 'local%04d.txt' % file_num, FILE_CONTENT)
        # create  folder a2
    ...
    def _move_and_copy_paste_folder(self, folder_1, folder_2, target_folder, stopped=True):
        ...
        assert self.local_client_1.exists(new_path)
        ...
        good = set(['local%04d.txt' % file_num
                    for file_num in range(1, self.NUMBER_OF_LOCAL_FILES + 1)])

        # expect '/a2/a1' to contain the files
        assert os.path.exists(self.local_client_1.abspath(new_path))
        children_1 = os.listdir(self.local_client_1.abspath(new_path))
        assert len(children_1) == self.NUMBER_OF_LOCAL_FILES
        assert set(children_1) == good
        
        # expect 'Nuxeo Drive Test Workspace/a1' to contain also the files
        assert os.path.exists(self.local_client_1.abspath(copy_path))
        children_2 = os.listdir(self.local_client_1.abspath(copy_path))
        assert len(children_2) == self.NUMBER_OF_LOCAL_FILES
        ...
        assert set(children_1_name) == good
        assert len(children_2) == self.NUMBER_OF_LOCAL_FILES
        children_2_name = set()
        for child in children_2:
            children_2_name.add(child.name)
        assert set(children_2_name) == good
    ...
    @pytest.mark.skipif(
        sys.platform == 'linux2',
        reason='NXDRIVE-471: Not handled under GNU/Linux'
               ' as creation time is not stored.')
