...
class EngineDAOTest(TestCase):

    @staticmethod
    def _get_default_db(name='test_engine.db'):
    ...
    def test_init_db(self):
        ...
        # Test configuration
        assert not dao.get_config('remote_user')
    ...
    def test_migration_db_v1_with_duplicates(self):
        ...
        assert not self._dao.get_config('remote_last_event_log_id')
        assert not self._dao.get_config('remote_last_full_scan')
    ...
    def test_errors(self):
        ...
        assert not row.last_error_details
    ...
    def test_batch_folder_files(self):
        ...
        while index > 0:
            ...
            assert state.id == ids[index]
        assert not self._dao.get_previous_folder_file(state.remote_ref)
        
        # Last file is 9
        state = self._dao.get_state_from_id(46)
        assert not self._dao.get_next_folder_file(state.remote_ref)
    ...
    def test_batch_upload_files(self):
        ...
        while index > 0:
            ...
            assert state.id == ids[index]
        assert not self._dao.get_previous_sync_file(state.remote_ref, Engine.BATCH_MODE_UPLOAD)
        
        # Last file is 9
        state = self._dao.get_state_from_id(9)
        assert not self._dao.get_next_sync_file(state.remote_ref, Engine.BATCH_MODE_UPLOAD)
