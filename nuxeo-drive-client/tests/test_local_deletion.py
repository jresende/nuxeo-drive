...
class TestLocalDeletion(UnitTestCase):
    ...
    def test_untrash_file(self):
        ...
        self.wait_sync()
        assert self.remote_document_client_1.exists('/File_To_Delete.txt')
        ...
        assert not self.remote_document_client_1.exists('/File_To_Delete.txt')
        assert not self.local_client_1.exists('/File_To_Delete.txt')
        ...
        self.wait_sync(wait_for_async=True)
        assert self.remote_document_client_1.exists(old_info.uid)
        assert self.local_client_1.exists('/File_To_Delete.txt')
    ...
    def test_untrash_file_with_rename(self):
        ...
        self.wait_sync(wait_for_async=True)
        assert self.remote_document_client_1.exists(old_info.uid)
        assert self.local_client_1.exists('/File_To_Delete2.txt')
        assert not self.local_client_1.exists('/File_To_Delete.txt')
        assert self.local_client_1.get_content('/File_To_Delete2.txt') == 'New content'
    ...
    def test_move_untrash_file_on_parent(self):
        ...
        self.wait_sync()
        assert self.remote_document_client_1.exists(file_path)
        ...
        self.wait_sync(wait_for_async=True)
        assert not self.remote_document_client_1.exists(file_path)
        assert not self.local_client_1.exists(file_path)
        # See if it untrash or recreate
        shutil.move(os.path.join(self.local_test_folder_1, 'File_To_Delete.txt'), self.local_client_1.abspath('/'))
        self.wait_sync(wait_for_async=True)
        ...
        assert self.local_client_1.get_remote_id('/').endswith(new_info.parent_uid)
    ...
    def test_move_untrash_file_on_parent_with_no_rights(self):
        ...
        # See if it untrash or recreate
        shutil.move(os.path.join(self.local_test_folder_1, 'File_To_Delete.txt'), self.local_client_1.abspath('/'))
        assert self.local_client_1.get_remote_id('/File_To_Delete.txt')
        self.wait_sync(wait_for_async=True)
        assert self.local_client_1.exists('/File_To_Delete.txt')
    ...
    def test_untrash_file_on_delete_parent(self):
        ...
        assert self.remote_document_client_1.exists(old_info.uid)
        new_info = self.remote_document_client_1.get_info(old_info.uid, use_trash=True)
        assert self.remote_document_client_1.exists(new_info.parent_uid)
        assert self.local_client_1.exists(file_path)
