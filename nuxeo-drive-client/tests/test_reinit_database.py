...
class TestReinitDatabase(UnitTestCase):
    ...
    def _reinit_database(self):
        ...
        self.engine_1 = self.manager_1.bind_server(
            self.local_nxdrive_folder_1, self.nuxeo_url,
            self.user_1, self.password_1, start_engine=False)
    ...
    def _check_conflict_automatic_resolution(self):
        assert not self.engine_1.get_dao().get_conflicts()
    ...
    def test_synchronize_remote_change(self):
        ...
        assert self.local.get_content('/Test folder/Test.txt') == 'This is some content'
    ...
    def test_synchronize_local_change(self):
        ...
        assert self.remote.get_content('/Test folder/Test.txt') == 'This is some content'
    ...
    def test_synchronize_remote_and_local_change(self):
        ...
        assert self.local.get_content('/Test folder/Test.txt') == 'Content has locally changed'
        assert self.remote.get_content('/Test folder/Test.txt') == 'Content has remotely changed'
